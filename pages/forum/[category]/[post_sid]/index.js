...
export default function ReadPost({ src = '', post }) {
  ...
  useEffect(() => {
    fetch(process.env.API_SERVER + '/' + router.query.post_sid)
      .then((r) => r.json())
      .then((result) => {
        if (result.success) {
          setRow(result.row)
        ...
        <div className={`${styles.row}`}>
          ...
          {row && (
            <div>
              ...
              <div>·</div>
              <div className={`${styles.adjust}`}>{row.postcategory_sid}</div>
              <div>·</div>
              <div className={`${styles.adjust}`}>{row.publish_time}</div>
            </div>
          )}
          {/* </div>
          ))} */}
